...
from aos.adapters.africas_talking.ussd_adapter import USSDAdapter
...
@pytest.fixture
def ussd_adapter_test(ussd_gateway, session_manager):
...
@pytest.mark.asyncio
async def test_ussd_main_menu_display(ussd_adapter_test, ussd_gateway):
    ...
    request = ussd_adapter_test.parse_request(payload)
    response = await ussd_adapter_test.handle_request(request)
...
@pytest.mark.asyncio
async def test_ussd_harvest_flow_complete(ussd_adapter_test, ussd_gateway):
    ...
    # Without AgriModule, it returns "Finalized"
    assert "Finalized" in response.content
...
def test_ussd_session_cleanup(session_manager):
    ...
    session.updated_at = datetime.now(UTC) - timedelta(seconds=400)
...
def test_ussd_response_formatting(ussd_adapter_test):
    ...
    formatted = ussd_adapter_test.format_response(response)
