...
class DomainRouter:
    ...
    async def route_command(self, chat_id: int, command: str, args: List[str]) -> bool:
        ...
        if not active_domain_name:
            ...
            await self.show_domain_selector(chat_id)
            return True
        ...
        if domain:
            ...
            handled = await domain.handle_command(chat_id, command, args)
            if not handled:
    ...
    async def route_callback(self, chat_id: int, callback_data: str) -> bool:
        ...
        if callback_data.startswith('switch_domain_'):
            ...
            await self.adapter.send_message(str(chat_id), f"✅ Switched to <b>{display_name}</b> domain.")
            # Show domain-specific help
            await self.show_domain_help(chat_id, domain_name)
            return True
        ...
        domain = self.domains.get(active_domain_name)
        if domain:
            return await domain.handle_callback(chat_id, callback_data)
            
        return False

    async def show_domain_selector(self, chat_id: int):
        ...
        if not user:
            ...
            await self.adapter.send_message(str(chat_id), "Welcome! Please use /register to create your profile first.")
            return
        ...
        await self.adapter.send_message(str(chat_id), "📂 <b>Select Active Domain</b>\n\nChoose the service you want to use right now:", metadata={"reply_markup": keyboard})

    async def show_domain_help(self, chat_id: int, domain_name: str):
        ...
        await self.adapter.send_message(str(chat_id), help_text)
